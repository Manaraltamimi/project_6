...
contract SupplyChain is Ownable, FarmerRole, DistributorRole , RetailerRole, ConsumerRole{
  ...
  function packItem(uint _upc) verifyCaller(items[_upc].originFarmerID) processed(_upc) public
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  function purchaseItem(uint _upc) verifyCaller(items[_upc].consumerID) received(_upc)public
    // Call modifier to check if upc has passed previous supply chain stage
  ...
  )
  {
    // Assign values to the 9 parameters
        ...
        state = uint(items[_upc].itemState);
